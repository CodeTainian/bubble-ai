...
const submit = async () => {
  saving.value = true
  try {
    const res = isAdmin.value ? await updateAppByAdmin({ id, appName: form.appName, cover: form.cover, priority: form.priority }) : await updateApp({ id, appName: form.appName })
