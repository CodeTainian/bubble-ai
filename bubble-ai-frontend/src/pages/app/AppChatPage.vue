    ...
    <main ref="workspace" class="workspace" :class="{ resizing: resizingPane }" :style="{ gridTemplateColumns }">
      <section class="conversation">
        ...
        <div class="composer-wrap">
          <div class="composer">
            <a-textarea v-model:value="input" :bordered="false" :auto-size="{ minRows: 3, maxRows: 5 }" placeholder="描述得越详细，页面越具体。可以继续提出修改意见..." @pressEnter="handleEnter" />
            <div class="composer-footer">
              <span><MessageOutlined /> 继续对话完善页面</span>
              <a-button type="primary" shape="circle" :loading="generating" :disabled="!input.trim()" @click="sendMessage"><ArrowUpOutlined /></a-button>
            </div>
          </div>
        </div>
      </section>
...
const send = (content: string) => {
  ...
  eventSource?.close()
  eventSource = new EventSource(`http://localhost:8123/api/app/chat/gen/code?appId=${id}&message=${encodeURIComponent(content.trim())}`, { withCredentials: true })
  eventSource.onmessage = (event) => {
    assistant.pending = false
    assistant.content += normalizeChunk(event.data)
    scrollToBottom()
  }
  eventSource.addEventListener('done', finishGeneration)
  eventSource.onerror = () => finishGeneration()
  scrollToBottom()
}
...
const finishGeneration = () => {
  ...
  if (assistant?.role === 'assistant') {
    assistant.pending = false
    if (!assistant.content) assistant.content = '页面已经生成完成，可以在右侧查看效果。'
  }
...
.conversation, .preview-pane, .version-bar { background: #fff; }.conversation { display: flex; min-height: 0; flex-direction: column; }.message-list { flex: 1; overflow-y: auto; padding: 18px 15px; }
...
.composer-wrap { padding: 12px; }.composer { padding: 9px 10px; border: 1px solid #e1e8e8; border-radius: 17px; background: #fff; box-shadow: 0 7px 24px rgba(29,90,94,.09); }.composer textarea { resize: none; }.composer-footer, .preview-toolbar { display: flex; align-items: center; justify-content: space-between; }.composer-footer span { color: #a0abad; font-size: 12px; }
