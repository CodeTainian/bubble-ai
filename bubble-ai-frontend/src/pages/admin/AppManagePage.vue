<template>
  <div class="app-manage-page">
    ...
    <a-form layout="inline" :model="searchParams" @finish="search">
      <a-form-item label="应用 ID"><a-input v-model:value="searchParams.id" placeholder="输入 ID" /></a-form-item>
      <a-form-item label="应用名称"><a-input v-model:value="searchParams.appName" allow-clear placeholder="输入应用名称" /></a-form-item>
      <a-form-item label="用户 ID"><a-input v-model:value="searchParams.userId" placeholder="输入用户 ID" /></a-form-item>
      <a-form-item label="生成类型"><a-input v-model:value="searchParams.codeGenType" allow-clear placeholder="如 html" /></a-form-item>
...
<style scoped>
.page-title h2 { margin-bottom: 5px; font-size: 26px; }.page-title p { margin-bottom: 24px; color: #8a999b; }.muted { color: #b6bfc0; }
</style>
