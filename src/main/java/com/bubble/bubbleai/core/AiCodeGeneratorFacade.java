...
public class AiCodeGeneratorFacade {
    ...
    public Flux<String> generateAndSaveCodeStream(String userMassage, CodeGenTypeEnum codeGenTypeEnum,
        ...
        return switch (codeGenTypeEnum){
            case HTML -> {
                ...
                yield propagateMonitorContext(
                        processCodeStream(codeStream,CodeGenTypeEnum.HTML,appId), monitorContext);
            }
            case MULTI_FIlE -> {
                Flux<String> codeStream = aiCodeGeneratorService.generateMultiFileCodeStream(userMassage);
                yield propagateMonitorContext(
                        processCodeStream(codeStream,CodeGenTypeEnum.MULTI_FIlE,appId), monitorContext);
            }
            case REACT_PROJECT -> {
    ...
    private Flux<String> processTokenStream(TokenStream tokenStream, MonitorContext monitorContext) {
        return Flux.create(sink->{
            ...
            // Flux subscription runs on a worker thread. Restore the captured
            // request context only around start(); the model wrapper propagates
            // it to asynchronous response/tool callbacks and clears each scope.
